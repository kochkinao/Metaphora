...
async def select_card1(msg: types.Message):
    ...
    with open(photo.ryb_al, 'rb') as ryb_al:
        await bot.send_photo(msg.from_user.id, caption='Выберите карту из колоды', photo=ryb_al, reply_markup=kb.num)
...
async def select_card2(msg: types.Message):
    ...
    with open(photo.ryb_radost, 'rb') as ryb_radost:
        await bot.send_photo(msg.from_user.id, caption='Выберите карту из колоды', photo=ryb_radost, reply_markup=kb.num)
...
async def select_card3(msg: types.Message):
    ...
    with open(photo.ryb_strah, 'rb') as ryb_strah:
        await bot.send_photo(msg.from_user.id, caption='Выберите карту из колоды', photo=ryb_strah, reply_markup=kb.num)

async def last_answer(msg:types.message, state:FSMContext):
    await state.update_data(question7 = msg.text)
    if mes == 'Не хочу отправлять':
        await msg.answer(f'Вы смогли ответить на свой вопрос?',reply_markup=kb.yes_no)
        await UserState.question8.set()
    else:
        await msg.answer(f'Вы смогли ответить на свой вопрос?\nВопрос: "<i>{mes}</i>"', parse_mode = "HTML", reply_markup=kb.yes_no)
        await UserState.question8.set()
...
def register_handler_where_are_you_able_to_answer(dp: Dispatcher):
    ...
    dp.register_message_handler(last_answer, state=UserState.question7)